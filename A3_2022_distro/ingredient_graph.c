...
int ingredient_index(char source_ingredient[MAX_STR_LEN])
{
    ...
    for (int i = 0; i < MAT_SIZE; i++) {
      if (!strcmp(source_ingredient, ingredients[i])) return i;
    }
    return -1;
}
...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
     ...
     * Implement this function
     * ********/
    int ind = ingredient_index(source_ingredient);
    for (int i = 0; i < MAT_SIZE; i++) {
      if (ind != i && AdjMat[ind][i]) printf("%s\n", ingredients[i]);
    }

}

int in_list(intNode* h, int ind) {
  while(h != NULL) {
    if (h->x == ind) {
      return 1;
    }
...
intNode* deleteValue(intNode* h, int v) {
  if (h->x == v) {
    return h->next;
  }
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
     ...
     *******/
    int ind = ingredient_index(source_ingredient);
    if (searchInt(h, ind) || dist > k) {
      return h; //Nothing happens
    }

    if (dist) h = insertInt(h, ind);

    for (int i = 0; i < MAT_SIZE; i++) {
      if (AdjMat[ind][i] > 0) h = related_k_dist(h, ingredients[i], k, dist+1);
    }
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
     ...
     * Implement this function
     *****/
    intNode *related = NULL, *avoided = NULL;
    related = related_k_dist(related, source_ingredient, k_source, 0);
    avoided = related_k_dist(avoided, avoid, k_avoid, 0);

    int avoidind = ingredient_index(avoid);

    // Delete aviod
    if (searchInt(related, avoidind)) {
      related = deleteValue(related, avoidind);
    }

    // Set of only r, so delete neighbors of r
    for (intNode* ptr = avoided; ptr != NULL; ptr = ptr->next) {
      if (searchInt(related, ptr->x)) {
        related = deleteValue(related, ptr->x);
      }
    }
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
  ...
  int ind, change_ind;
  for (int i = 0; i < 10; i++) {
    if (!strcmp(recipe[i], to_change)) {
      change_ind = i;
      break;
    }
  }

  for (int i = 0; i < 10 && strcmp(recipe[i],""); i++) {
    if (i == change_ind) {
      continue;
    }

    ind = ingredient_index(recipe[i]);
  ...
  strcpy(recipe[change_ind], ingredients[maxi]);
  //printf("%s\n", ingredients[maxi]);
}
