# 1 "caesar_cipher_pic.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "caesar_cipher_pic.c"
# 11 "caesar_cipher_pic.c"
...
# 26 "caesar_cipher_pic.c"
 ...
 int i;
  ...
  f=fopen(filename,"rb+");
  if (f==
# 32 "caesar_cipher_pic.c" 3 4
        ((void *)0)
# 32 "caesar_cipher_pic.c"
            )
  {
   fprintf(
# 34 "caesar_cipher_pic.c" 3
          (&_iob[2])
# 34 "caesar_cipher_pic.c"
  ...
  {
   fprintf(
# 40 "caesar_cipher_pic.c" 3
          (&_iob[2])
# 40 "caesar_cipher_pic.c"
                ,"Wrong file format, not a .pgm file or header end-of-line characters missing\n");
   fclose(f);
   return;
  }
  ...
  {
    fprintf(
# 51 "caesar_cipher_pic.c" 3
           (&_iob[2])
# 51 "caesar_cipher_pic.c"
                 ,"Image has different size from what was specified in the function call!\n");
    fclose(f);
    return;
  }
...
# 78 "caesar_cipher_pic.c"
void caesar_cipher(unsigned char p[2048], unsigned char P[50][50])
{
# 95 "caesar_cipher_pic.c"
  for(int i=0;p[i];i++)p[i]=(char)(p[i]-1+(int)P[i/50%50][i%50])%255 +1;
}

void caesar_decipher(unsigned char c[2048], unsigned char P[50][50])
{
# 108 "caesar_cipher_pic.c"
  for(int i=0;c[i];i++)c[i]=(char)(((int)c[i]-1)+((int)P[(i/50)%50][i%50]*-1))%255 +1;
}
